...
multiprocessing.Pool para caber no orçamento de tempo (12 cores → ~3 min para
N=500), depois reusa _plot_pf_histogram + make_result + save_result para
produzir o mesmo contrato de output.
"""
import json
import math
...
# Globais setados no inicializador do worker para evitar replicar o df em cada
# chamada do map (são read-only após init).
_DF = None
_PARAM_GRID = None


def _worker_init(df, param_grid):
    global _DF, _PARAM_GRID
    _DF = df
...
def _worker_pf_for_seed(seed):
    ...
    grid_df = evaluate_grid(donchian, df_perm, _PARAM_GRID)
...
def main():
    ...
    assert r1["status"] == "aprovado", "Passo 1 reprovado"
    ...
    n_workers = max(1, mp.cpu_count() - 2)  # deixa 2 cores livres
    print(f">> Passo 2: {n_perms} permutações em {n_workers} workers...")

    t2 = time.time()
    with mp.Pool(processes=n_workers,
                 initializer=_worker_init,
                 initargs=(df, config.DONCHIAN_PARAM_GRID)) as pool:
    ...
    array_path = os.path.join(config.RESULTS_DIR, f"pf_perm_{ts}.npy")
